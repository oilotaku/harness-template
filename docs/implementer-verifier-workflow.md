...
## 分離擋不住的一類：相關性誤解（同一個模型的盲點）

上面那三種問題，根源都是「**同一個 agent 既當球員又當裁判**」——分離就能解。
但分離解不掉另一類問題，這裡誠實記下來，因為它跟「同 OS 使用者」並列為本模板
兩個最需要知道的限制：

**當 implementer 與 verifier 是同一個底層模型時，他們的錯誤是相關的。**

隔開 session、不共用上下文，能讓兩邊**不互相污染判斷**；但它換不掉兩邊
**共有的先驗**。一段有歧義的 task-spec，同一個模型很可能在兩邊都朝同一個方向誤讀——
verifier-test-writer 依那個誤讀寫測試，implementer 依同一個誤讀寫程式，於是測試綠燈、
隱藏測試也綠燈，而**整件事一起錯了**。隱藏測試防的是「作弊」（明知規格卻取巧），
它防不了「雙方都真心以為規格是這個意思」。

這不是靠再加一層機制能解的，能降低它的是三件事，由風險決定用到哪一層：

1. **task-spec 越明確，相關性誤解的空間越小**——這是最高 CP 值的一招，也是
   黃金法則第 4、7 條與 `docs/token-strategy.md` §2.1 反覆強調 spec 自足的另一個理由。
   歧義是相關性誤解的燃料，把它寫掉最划算。
2. **盡量讓 implementer 與 verifier 用不同的模型**，把「共有先驗」這件事本身拆開。
   具體怎麼指派見 [`model-thinking-matrix.md`](model-thinking-matrix.md)
   的「降低相關性誤解」一節。
3. **規格模糊或 L3/L4 的任務，加一道人類複審規格**：在 verifier-test-writer 動工前，
   讓人確認「這份 spec 只有一種讀法」。人是目前唯一跟兩個 Claude 都不相關的判斷來源。

判斷準則：task-spec 一有歧義就停下來問（黃金法則第 7 條），不要讓兩邊各自
（很可能一致地）猜一個版本繼續做——那正是這類失效最典型的起點。
