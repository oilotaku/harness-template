...
### P0-3. Bash 分支有一整排結構性繞過【已驗證】✅ 已修正（先界定範圍再做）
...
> **實際落地**：先把題目**重新界定成有界的**，再做掉。原本的形狀是「把所有繞過
> 寫法都擋掉」——列舉式防守對上無限攻擊面，沒有終點（`scripts/estimate-time.py`
> 會直接拒絕估這種任務，見 `docs/time-estimation.md` §1）。上面草案的第二點
> 就是那個形狀：補一份更長的動詞清單，下一種包裝出現時再補一次。
>
> 界定後的問題只剩一個，而且是有界的：
>
> > 這段 Bash 指令裡，有沒有任何一段**解析得出受保護的隱藏測試路徑**？有就擋。
>
> 這把問題從「動詞是什麼」換成「有沒有指到那個位置」，一次涵蓋表格列出的六類
> 寫法（`bash -c`、`find -delete`、`| xargs`、`tar`、`$(…)`、變數展開），
> 不必逐一補動詞。跟封存庫（P1-1）用的是同一條規則：**不分動詞**。
>
> 刻意**不是**子字串比對：範例目錄（`templates/examples/…`）底下那個同名路徑
> 並不在受保護路徑內，子字串比對會誤傷它。字面片段只用來定位，是否受保護仍由
> 路徑正規化決定，所以範例目錄與 `cd` 進去之後的相對寫法都不會被擋
> （各有一個「不可誤傷」的回歸測試）。
>
> **明確不在範圍內**：完全不寫出路徑的混淆（base64 解碼、逐字元組字串、
> 從檔案讀出路徑再用）。那才是無限的攻擊面，這一層不試圖窮盡——穿過去也只會
> 讀到密文。這句話寫進了 guard 的模組說明與 README：**這一層是縱深防禦，
> 不是主防線。**
>
> **已知的代價**：指令字串裡「提到」這個路徑就會被擋，即使只是想把它寫進某個
> 檔案的內容（寫測試、寫文件都會踩到——這份文件本身就是用 Write/Edit 改的）。
> 那條路請用 Write/Edit 工具，它只看目標檔案不看內容；拒絕訊息裡直接寫了這個
> 出口。多擋一點是安全的，少擋一點才危險。
>
> **一個刻意翻轉的預期**：舊測試寫著「用 `unittest discover` 直接跑暫存區不該
> 被擋」，那是 P1-1 之前的假設（當時暫存區就是存放處，擋掉等於沒人跑得了）。
> 封存機制上線後唯一的合法入口是 `run-hidden-tests.py`（需要權杖），而直接跑
> 暫存區會把通過/失敗變成可反覆查詢的 oracle。該案例已改成預期被擋，
> 並在註解裡寫明為什麼翻轉。
>
> 回歸測試 `test-guards.py` 66 → 77：六類繞過各一個案例，加上三個「不可誤傷」
> 案例（範例目錄、`cd` 之後的相對路徑、讀取公開測試）。
...
> **實際落地**：三支掃描腳本與 `init.py` 都加了 `--json`（stdout 只有 JSON），
> 並依實作過程中發現的問題做了幾處刻意的偏離：
>
> - **欄位改名為 `max_parallel_agents`**（草案是 `recommended_parallel_agents`）。
>   機器跑得動不等於應該開平行：在訂閱制方案下平行度會等倍放大用量視窗的消耗速率
>   （見 `token-strategy.md` §3.4）。所以 JSON 同時給 `default_parallel_agents`，
>   而且它恆為 1——上限與建議值是兩件事，命名不該混為一談。
> - **換算邏輯抽成純函式** `scripts/capacity.py`，才測得到邊界：
>   讀不到 CPU 核心數時保守回 1（猜大值的代價遠高於猜小值）、
>   記憶體不足時由記憶體決定、極小機器至少回 1。
> - **掃描不完整時 `suggested_port_range` 一律為 null**（另有 `complete` 旗標）。
>   沒裝 psutil 或權限不足時，「沒看到」不等於「沒被佔用」，
>   給一個看起來很有把握的區間比不給更危險。
> - **快取（`.harness/last-scan.json`）只認能力指紋**：`capability_key` 不符
>   （換機器、資源級距變了）就一律回 None，不回半套資料。hostname 不列入，
>   否則容器/CI 每次重建都會讓快取失效（那正是 P3-1 修掉的誤報來源）。
>   「是不是同一個 session」腳本看不到，由 Orchestrator 判斷，快取只提供 `age_seconds`。
> - `.harness/progress/` 之外，`.harness/` 仍不接受一般編輯寫入；
>   快取是由掃描腳本自己寫的，不經過工具呼叫，因此不受 guard 影響。
>
> 回歸測試新增 `scripts/test-scan-json.py`（32 案例），其中一條守的是實作過程中
> 真的踩到的 bug：第一版解析連接埠的 regex 是 `[:.](\d{1,5})\b`，
> 於是 `127.0.0.1:33697` 會被解析出 0、0、1、33697 四個「埠號」。
> CI 另外加了一步「`init.py --json` 在三平台都必須是合法 JSON」——
> 人看的模式跑得起來，不代表 `--json` 沒有混進中文。
