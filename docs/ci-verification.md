...
## 1.5 什麼時候該開（以及什麼時候它不是選配）

`ci_verification` 預設是 `false`，因為它需要 GitHub、需要有人把密文包 commit
進預設分支、把權杖存成 secret——對「自己一台機器、implementer 不是對手」的情境
是多餘的負擔。但它的預設是「關」不代表它是「進階選項」；它是**唯一跨得過本機
結構性上限的那一步**。用這條準則判斷：

| 你的情境 | CI 驗收 |
|---|---|
| 隱藏測試只是要擋「順手看到」的非刻意洩題，implementer 不是對手 | 選配。本機四層就夠 |
| implementer 可能**決心繞過**、而且跟 verifier 共用同一個 OS 使用者 | **不是選配，是必須**——本機任何一層在這個威脅模型下都有那個換不掉的上限（§1） |
| 驗收結果會被拿去做有代價的決定（發布、合規、對外承諾） | 強烈建議——本機那一跑無法對第三方證明沒有被動過手腳 |

換句話說：**本機那套適合「防誤讀」，CI 那套才適合「防對手」。** 兩者威脅模型不同，
不是強度高低之分。開了之後本機那一跑不會消失，它降為「第二意見」——兩邊不一致
要當可疑訊號，不是環境差異（見 `implementer-verifier-workflow.md`）。
