...
| `feedback` | 某個 bug 的**根因反映流程或規格層級的問題**（不是單純寫錯）。這是 `docs/root-cause-and-fix.md` 步驟 7 指過來的那一列 | 「這個專案的 task-spec 常常沒寫清楚錯誤處理邊界，之後拆這類 task 要多問一輪」 |
| `feedback` | 同一類**使用者回報**反覆出現（見 `docs/user-reports.md`）——特別是反覆的**誤用**，那通常是文件或介面的問題，不是使用者的問題 | 「使用者常把匯出當成備份用，之後這類功能的錯誤訊息要寫得更明確」 |
| `project` | 使用者回報揭露的**使用脈絡**：他們實際在什麼環境、怎麼用這個系統 | 「主要使用者跑在 Windows 且沒有 Docker，驗收要涵蓋這個組合」 |
...
- **不要**把 task-spec、驗收報告、回報單的內容複製進記憶——記憶只該存「文件裡沒寫、
  但下次拆解還用得到的判斷依據」，不是任務產出本身的備份。
  一份回報單該留下的不是「他回報了什麼」，而是「這件事改變了我對這個專案的什麼判斷」。
