...
- **task_id**：
- **設計基準**：（只有牽涉畫面的 task 要填：`模板預設` / `專案自訂` ——
  由 Orchestrator 問過使用者後填，見 `docs/frontend-design-defaults.md` §2）
...
- **容器化**：（`不使用` / `Docker` / `沿用既有容器環境`——由 Orchestrator 問過使用者後填，
  見 `.claude/agents/orchestrator.md` 需求釐清那一步。implementer 不得自行決定）
  - 若為 Docker：基底映像 ______、對外發佈的埠 ______、需要持久化的資料 ______、
    開發時的啟動方式 ______
- **可用連接埠**：
- **需避開的既有服務**：（含既有的 compose 服務名稱與埠號）
