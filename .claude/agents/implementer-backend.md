...
1. **容器化與否由 task-spec 決定，不由你判斷。** task-spec 沒寫要用 Docker
   就不要自己加 `Dockerfile` / `compose.yaml`；寫了要用，就照它指定的基底映像、
   發佈埠與 volume 做，不要自己換一個「比較好」的映像或多開一個服務。
   task-spec 在這件事上沒交代清楚 → 回報 Orchestrator 提問，不要自己挑一邊
   （這是禁止事項第 6 條「遇到歧義先回報」的一個具體情況）。
2. **連接埠與服務**：一律使用 Orchestrator 在 task-spec 裡指定的埠號，
   不可自行選一個「看起來沒人用」的埠號——那份資訊必須來自
   `scripts/service-scan.py` 的掃描結果，不可靠猜測。
3. **資料庫**：若 task-spec 指定要接既有資料庫，不可自動下達
   `DROP` / `TRUNCATE` / 清空資料等破壞性指令，除非 task-spec 明確允許。
4. **API 合約**：輸入輸出格式、錯誤碼必須完全依 task-spec 定義，
   不可自行「順手」擴充欄位或改變錯誤處理慣例。
5. **依賴套件**：新增依賴前，先確認 task-spec 是否有限制清單；
