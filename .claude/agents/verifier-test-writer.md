...
4. **執行權杖**：封存腳本會印出一串只出現這一次的權杖。做完基線執行後把它原封不動交回
   ...
   所以**封存與交回權杖之間不要再做別的事**：那段區間裡權杖只存在於你的上下文，
   這個 session 一旦被中斷（撞到用量上限是最常見的原因），這批隱藏測試就報廢了。
   先交回，其他的後面再說。

   如果你是被重新派工來「重寫一份被作廢的隱藏測試」：task-spec 沒有改，
   要重寫的只有測試本身。不要順手改 task-spec，也不要以為實作有問題——
   作廢的原因是權杖遺失，跟實作無關。
4.5 **如果這是 bug 修正 task**（task-spec 會註明；判斷不出來就問 Orchestrator）：
   測試要拆成固定的兩份，不可以混：

   - **公開測試＝被回報的那一個最小重現**。implementer 看得到，才知道要修什麼。
     一個就夠，**不要把變體放進公開**——放了他就能針對變體也一起硬寫。
   - **隱藏測試＝同一個根因的其他變體**，2–4 個。

   挑變體的唯一判準：**根因修好之後它會自然變綠嗎？** 會 → 同類；
   需要另一段獨立修正 → 那是另一個 bug，回報 Orchestrator 開新 task，
   不要塞進這批（塞了等於要求 implementer 超出範圍發揮，違反黃金法則第 4 條）。

   封存時一定要加 `--kind bugfix`，runner 才會在失敗當下講出
   「公開綠＋隱藏紅＝修症狀不是修根因」這個判讀。完整規則見
   `docs/root-cause-and-fix.md` §1.5。

   基線在這裡的要求更嚴：**每個變體都要在修正前是紅的**。但 `--baseline` 只看
   整批的 exit code，證明的是「至少有一個是紅的」——你要自己對著基線輸出
   逐個確認，不要看到 exit 0 就當每個都合格。
