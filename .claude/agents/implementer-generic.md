...
5. **不可以改版本檔**（`VERSION`、`package.json` 的 `version`、`pyproject.toml` 的
   `version`——實際位置由 `harness.config.json` 宣告）。版本是治理資訊，由
   Orchestrator 用 `scripts/version.py` 升。**讀它沒有問題**（例如實作 `--version`
   旗標），只有寫入會被 guard 擋下；被擋到不是故障，見 `docs/versioning.md` §3。
6. 遇到 task-spec 有歧義、或跟現有程式碼庫衝突，**先回報 Orchestrator 提問**，
   不要自己猜一個答案就繼續寫。
7. **測試失敗時走 RCA 流程**：先穩定重現、定位到一行、問「為什麼沒被更早抓到」，
   再最小幅度修正（`docs/root-cause-and-fix.md`）。禁止跳過測試、放寬斷言、
   或把失敗歸因為 flake 來讓燈變綠。
8. **停損規則**：如果同一條驗收標準連續失敗 ≥3 次，或發現非得修改
