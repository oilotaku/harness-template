...
| `scripts/version.py` | **產出專案的版本號**：讀寫 semver，支援純文字 / `package.json` / `pyproject.toml` 三種來源，寫回去不破壞檔案其他內容。只有 Orchestrator 能用，implementer 對版本檔的寫入會被 guard 擋下（讀不擋） | 拆解任務前確認版本、一批需求驗收完成後升版 |
