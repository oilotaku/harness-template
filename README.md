...
**那把「密」是什麼等級**（避免誤用）：封存用的是只靠 Python 標準函式庫實作的
SHA-256 keystream XOR（逐檔獨立金鑰），**不是 AES**。它的威脅模型就是上面那件事
——擋非刻意的洩題——屬**防誤讀等級，不是機密等級**。所以隱藏測試裡**不要放
真正敏感的東西**（憑證、個資、金鑰、商業機密）：那類資料該用作業系統權限或
專門的秘密管理，而不是這套封存。實作取捨（為什麼不用 AES、如何避開 two-time-pad）
見 [`docs/ci-verification.md`](docs/ci-verification.md) §1 與 `scripts/hidden_vault.py` 的模組說明。
